...
# ble_connection.send_message("ssid"+ssid + "\n")
# time.sleep(1)  # Give ESP32 time to process SSID
# ble_connection.send_message("pass"+password + "\n")

model_reaction_bot = genai.GenerativeModel("gemini-pro")
model_check = genai.GenerativeModel("gemini-pro")
...
You are supposed to talk like a natural human & your responses should be very very short and interesting.Try to be very very short while answering sometimes even a single word would be enough . you also do all the things that a alexa or google home does like playing music scheduling task, playing youtube and all.
...
def process_prompt(model, prompt, user_input):
    try:
        ...
        if expression.lower() == "idk" or expression.lower() == "ik" :
...
async def handle_input():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        while True:
            ...
            # prompt_for_check = "check if the given conversation is talking about some query or asking to do some action , action like play videeo or song in youtube, or play song . schedule a meet , asking for system info , or system things like turning on bluetooth wifi hotspots , locking device or similar kinda thing . or asking to take a note of something or asked to shake a head or something .you are supposed to look at the last user said and analyze what is he asking . if the conversation dosen't look like a usual chat or query , if its lookk like asking to do something with systen  then reply with '''ik'''  only ik .or if its normal conversastion chat aand query then '''idk''' where the prompt is : {} ." 
            # res = process_prompt(model=model_check, prompt=prompt_for_check, user_input=user_input)
            # print(res)
            if 'ik' == "iks":
                pass
                # print("doing task")
                # task_incheck = model_check.generate_content("You are a ai assistant like alexa , but also like vector 2.0 .well you say what kind of task is that . you can do everything that alexa can   ")
            else :
